...
def p_program(p):  # Начало программы вызов переменных и тела проги
    ...
    p[0] = Node('Programm', [p[1], p[2]]
                )  # Для того, чтобы создать узел я создаю экземпляр класса Node
...
def p_identifier_list(p):  # перечисление переменных
    ...
    if len(p) == 2:
        # В этой продукции у меня как может создаваться узел, так и добавляться
        p[0] = Node("ID", [p[1]])
...
# Продукция для всего, может быть как пусто, так и все, циклы, функции, условия, выражения
...
def p_subprogram_head(p):  # Оглавление функции
    '''subprogram_head : FUNC ID arguments '''
...
# фигурные скобки и продукция - для всего, та самая сверху
