...
def add_eqstate(leaf1, leaf2, type_var, key='main'):
    ...
    if type_var == 'int':
        ...
        if leaf2 in list_of_registr:
            tac_dict[key].append(
                [tmp, '=', '_s' + str(list_of_registr.index(leaf2)+1)])
        ...
        if leaf2 in list_of_float:
            tac_dict[key].append(
                [tmp, '=', '_f' + str(list_of_float.index(leaf2)+1)])
...
def add_two_leaf(node, type_var, key='main'):
    ...
    if type_var == "int":
        ...
        if node.parts[0].parts[0] in list_of_registr:
            tmp_list = [tmp, '=', '_s' +
                        str(list_of_registr.index(node.parts[0].parts[0])+1)]
            tac_dict[key].append(tmp_list)
        else:
        ...
        if type(node.parts[2].parts[0]) == int:
            tac_dict[key].append(
                [tmp_1, '=', tmp, node.parts[1], node.parts[2].parts[0]])
            return tmp_1
        elif node.parts[2].parts[0] in list_of_registr:
            tac_dict[key].append(
                [tmp_1, '=', '_s'+str(list_of_registr.index(node.parts[2].parts[0])+1)])
            tmp_2 = init_var()
        ...
        if node.parts[0].parts[0] in list_of_float:
            tac_dict[key].append(
                [tmp, '=', '_f'+str(list_of_float.index(node.parts[0].parts[0])+1)])
        else:
            tac_dict[key].append([tmp, '=', node.parts[0].parts[0]])
        ...
        if node.parts[2].parts[0] in list_of_float:
            tac_dict[key].append(
                [tmp_1, '=', '_f'+str(list_of_float.index(node.parts[2].parts[0])+1)])
...
def add_while_or_if_goto(key_after, key_before, goto_key='global', condition=None):
    if condition is not None:
        if goto_key == "while":
            tac_dict[key_after].append(
                ["IfZ", condition[0], condition[1], condition[2], "Goto", key_before])
        else:
            tac_dict[key_after].append(
                ["If", condition[0], condition[1], condition[2], "Goto", key_before])
    else:
...
def add_func_call(key_after, part):
    tac_dict[key_after].append(
        ["LCall", "_" + part.parts[0], ret_func_args(part.parts[1])])
...
def recurs(tree, key_rec=key, key_after=''):
    ...
    for part in tree.parts:
        if type(part) != str and type(part) != int and type(part) != float:
            ...
            if part.type == "While":
                tmp_key = while_stmt(
                    part, key_after=key_rec, key_after_1=key_after)
                key_rec = tmp_key
                continue
            if part.type == "If\Else":
                tmp_key = if_else_stmt(
                    part, key_into=key_rec, key_after=key_after)
                key_rec = tmp_key
                continue
    ...
    for part in node.parts:
        ...
        if type(part) != str and type(part) != int and type(part) != float:
            ...
            if node.type == "Factor" and node.parts[0] == '(':
                ...
                if sad(part.parts[1]):
                    add_ret_func(key, simpl_expr(
                        part.parts[1], ret_fact(part.parts[1])))
                else:
                ...
                if sad(part.parts[2]):
                    add_eqstate(ret_fact(part.parts[0]), simpl_expr(
                        part, type_var, key), type_var, key)
                else:
                    add_eqstate(ret_fact(part.parts[0]), ret_fact(
                        part.parts[2]), type_var, key)
                ...
                if sad(part.parts[0]) and sad(part.parts[2]):
                    ...
                    variable = add_left_leaf(ret_fact(part.parts[0]), part.parts[1], simpl_expr(
                        part.parts[2], type_var, key), type_var, key)
                    return variable
                else:
                ...
                if sad(node.parts[0]) and sad(node.parts[2]):
                    ...
                    variable = add_right_leaf(simpl_expr(
                        part.parts[0], type_var, key), node.parts[1], ret_fact(node.parts[2]), type_var, key)
                    return variable
                elif sad(node.parts[2]):
                    variable = add_left_leaf(ret_fact(node.parts[0]), node.parts[1], simpl_expr(
                        node.parts[2], type_var, key), type_var, key)
                    return variable
                else:
    ...
    for part in node.parts:
        if part.type == 'expression':
            if part.parts[1] != ('and' or 'or'):
                ...
                if key_after_1 != '':
                    add_while_or_if_goto(
                        key_while, key_after_1, goto_key='while', condition=condition)
                else:
                    key_main = add_new_key("main")
                    key = key_main
                    add_while_or_if_goto(
                        key_while, key, goto_key='while', condition=condition)
            else:
        ...
        if part.type == 'expression':
            if sad(part.parts[0]) and sad(part.parts[2]):
                ...
                condition = add_exp_node(key_into,
                                         ...
                                         ret_fact(part.parts[2].parts[0]))
            add_while_or_if_goto(
                key_into, key_if, goto_key='if', condition=condition)
